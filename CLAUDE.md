...
워커는 항상 켜져있는 idle tmux 세션이다. 이 세션(오케스트레이터)에서 스크립트를 호출해 관리한다.
...
# 워커 1에 작업 전달
bash scripts/pool.sh send 1 "whos-life의 여행 기능 designing 해줘. spec은 /Users/hiyeop/dev/whos-life/work_logs/features/travel-planner/spec.md"
...
워커는 `01-org/engineering/worker.md` 역할이 자동 주입된 상태로 뜬다.
작업 할당은 `pool.sh send`로 메시지를 보내면 워커가 받아서 실행한다.
