...
- /Users/hiyeop/dev/memory-vault/01-org/core/planner/memory.md — 누적 학습 내용
...
`SPEC_READY` 출력 **직후** 반드시 아래 명령을 실행하라:

```bash
python3 ~/dev/memory-vault/scripts/notify.py \
  '{"event":"stage_complete","stage":"spec","task_id":<태스크ID>}' \
...
작업 중 배운 교훈, 반복 패턴, 실수는 `/Users/hiyeop/dev/memory-vault/01-org/core/planner/memory.md`에 기록한다.
