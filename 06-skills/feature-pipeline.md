...
| # | 섹션 | 내용 |
|---|------|------|
| 1 | **CLI Command Specifications** | 명령어, Input(argparse/Typer + Pydantic frozen), Output(text/--json), Logic, LLM Boundary, Error Cases |
| 2 | **DB Schema** | 실행 가능한 SQLite CREATE TABLE + INDEX |
| 3 | **Data Models** | Pydantic BaseModel 정의 (frozen=True) |
| 4 | **UI Screen Flow** | 화면별 데이터 소스(command), 사용자 액션 |
| 5 | **Agent Orchestration** | command 체이닝 순서, 판단 포인트, 에러 복구 |
| 6 | **Test Scenarios** | command별 테스트 케이스 (입력 → 기대 출력) |
| 7 | **LLM Boundary Summary** | spec.md Level 0/1/2를 command 단위로 매핑 |

#### CLI 표준 패턴

```python
# service.py — 순수 함수 (인터페이스 무관)
# service.py를 작성하면 통합 CLI가 자동으로 argparse를 생성한다.
def get_events(db: Any, params: GetEventsInput) -> dict:
    """이벤트 조회."""
    # Level 0: 코드로 100% 구현
    # Level 1: 내부에서 llm_call(task_type="structured") 호출
    # Level 2: 데이터 읽기/쓰기만. 판단은 Agent Orchestration에 기술
    ...

# 통합 CLI로 자동 노출 (개별 cli.py 불필요):
# whos-life feature {name} get-events --from 2026-02-23 --to 2026-02-28 --json
```

#### CLI 표준 규칙

```bash
# 통합 CLI (alias whos-life="python3 /Users/hiyeop/dev/whos-life/cli.py")
...
| Level 1 | `LLM: 정형 (함수 내부)` | 함수 내부에서 `llm_call(task_type="structured")` 호출. 모델은 config 기반 |
| Level 2 | `LLM: 에이전트 판단` | 데이터 함수만 제공, Agent Orchestration에 판단 로직 기술 |

> 모델명(Flash, Opus 등)은 SKILL.md에 쓰지 않음. 작업 유형(정형/비정형)만 명시하고 실제 모델 선택은 런타임 config.

**원칙**: SKILL.md는 "에이전트가 대화로 실행하는 흐름"이 아니라 **"빌드할 CLI command와 UI의 명세서"**.
...
| LLM 유틸 | `core/llm_utils.py` | 정형/비정형 LLM 동기 호출 (Level 1 함수 내부용) |
...
9. SKILL.md 작성 — **7개 섹션 규격 필수 준수** (위 "SKILL.md 형식 규격" 참조):
   - CLI Command Specifications (가장 중요)
...
### testing 단계 수행 절차 (Command-Level Verification)

> ⚠️ **금지**: 기능 전체를 사용자처럼 실행하지 않는다. 각 CLI command를 개별적으로 검증한다.

1. **DB Schema 검증** — SKILL.md의 CREATE TABLE SQL 실행, 테이블 생성 확인
2. **Command 단위 테스트** (핵심) — 각 CLI command 개별 검증:
   - SKILL.md Test Scenarios의 입력값으로 CLI 실행 (`--json` 출력 사용)
   - 반환값 vs 기대 출력 비교
   - Error case 처리 확인 (exit code + stderr JSON)
3. **Command 체이닝 통합 테스트** — Agent Orchestration 순서대로 데이터 흐름 검증
4. **UI 데이터 흐름 검증** — 화면별 데이터 소스 command 반환값이 UI에 충분한지 확인
5. **Codification 분석** — 각 command를 `codifiable` / `llm_required` / `hybrid`로 분류
6. **판정** — command 전체 통과 시 stable 승인 요청, 실패 시 designing 복귀 (decisions/ 기록)
7. `logs/test-{N}.md` 에 Command Test Results + Codification 분석 기록
8. `whos-life feature dev-queue record-test-run {id} --success --log "..."` 로 결과 기록
...
9. `codifiable` → service.py 순수 함수 구현 (통합 CLI가 자동 노출)
...
```bash
# alias whos-life="python3 /Users/hiyeop/dev/whos-life/cli.py"

# 아이디어 등록
whos-life feature dev-queue register-idea --title "제목" --description "설명" --category "카테고리"

# 인터뷰 내용 기록
whos-life feature dev-queue append-interview-log {id} --log "내용"

# 승인
whos-life feature dev-queue approve-to-queue {id}       # spec → queued
whos-life feature dev-queue approve-stable {id}         # testing → stable
whos-life feature dev-queue approve-plan {id}           # 계획 승인
whos-life feature dev-queue reject-plan {id} --reason "사유"  # 계획 거절

# 계획 제출
whos-life feature dev-queue submit-plan {id} --plan-json '{"plan": "..."}'

# 조회
whos-life feature dev-queue get-plan {id} --json
whos-life feature dev-queue get-pipeline-status --json   # 전체 현황
whos-life feature dev-queue get-queue-status --json      # 큐 상세

# 단계 전환
whos-life feature dev-queue advance-stage {id} --stage designing

# 테스트 결과 기록
whos-life feature dev-queue record-test-run {id} --success --log "결과"

# 알림 설정
whos-life feature dev-queue get-notify-settings --json
whos-life feature dev-queue set-notify-setting --stage designing --enabled

# 기타 기능 예시
whos-life feature token-monitor get-all-limits --json
whos-life feature portfolio-tracker get-holdings --json
whos-life feature hello-world greet --name "World" --json
```
...
### Command Test Results (Codification Analysis 앞에 위치)

```markdown
## Command Test Results
| Command | Test Case | Input | Expected | Actual | Status |
|---------|-----------|-------|----------|--------|--------|
| create-trip | 정상 생성 | --input '{"destination":"제주"}' | {"trip_id": 1} | {"trip_id": 1} | PASS |
| create-trip | 필수값 누락 | --input '{"destination":""}' | exit 1 + error JSON | exit 1 + error JSON | PASS |
```
...
testing 완료 시 마지막에 출력하는 JSON에 `command_results` 배열 포함:
...
```json
{
  ...
  "summary": "한 줄 요약",
  "command_results": [
    {"command": "create-trip", "tests_passed": 2, "tests_failed": 0},
    {"command": "search-places", "tests_passed": 1, "tests_failed": 1}
  ],
...
> 기존 `_parse_testing_result`는 `dict.get()` 사용하므로 `command_results` 추가에 하위 호환 유지.
...
whos-life/features/{category}/{name}/
  service.py            ← 핵심 로직 (순수 함수) → 통합 CLI 자동 노출
  render.py             ← UI

에이전트 → Bash → whos-life feature {name} {method} → service.py
